...
class HackSoundPlayer(GObject.Object, Loggable):
    ...
    def __init__(self, bus_name, sound_event_id, uuid_, metadata,
        ...
        Loggable.__init__(self, PlayerFormatter)
    ...
    def stop(self):
        ...
        try:
            ...
            self.error(ex)
            self.warning("Fade out effect could not be applied. Stop.")
    ...
    def __bus_message_cb(self, unused_bus, message):
        if message.type == Gst.MessageType.EOS:
            ...
            self.warning("Error from %s: %s (%s)", message.src, error, debug)
            self.pipeline.set_state(Gst.State.NULL)
...
class HackSoundServer(Gio.Application, Loggable):
    _TIMEOUT_S = 10
    ...
    def __init__(self, metadata):
        Gio.Application.__init__(self, application_id=self._DBUS_NAME,
                                 flags=Gio.ApplicationFlags.IS_SERVICE)
        Loggable.__init__(self, ServerFormatter)
        self._dbus_id = None
        self.metadata = metadata
    ...
    def ref(self, uuid_, bus_name):
        ...
        self.debug("Reference. Refcount: %d", self._refcount[uuid_][bus_name],
                   bus_name=bus_name,
    ...
    def unref(self, uuid_, bus_name, count=1):
        if uuid_ not in self._refcount:
            self.warning("This uuid is not registered in the refcount "
                         "registry.", uuid=uuid_)
            return
        if bus_name not in self._refcount[uuid_]:
            self.warning("Bus name '{}' is not registered in the "
                         "refcount registry.".format(bus_name),
                         bus_name=bus_name)
            return
        if self._refcount[uuid_][bus_name] == 0:
            self.warning("Cannot decrease refcount for bus name '{}'"
                         "because it's already 0.".format(bus_name),
                         uuid=uuid_)
            return
        self._refcount[uuid_][bus_name] -= count
        self.debug("Unreference. Refcount: %d",
    ...
    def _cancel_countdown(self):
        if self._countdown_id:
            ...
            self._countdown_id = None
            self.info('Timeout cancelled')

    def _ensure_release_countdown(self):
        self._cancel_countdown()
        self.hold()
        self.info('All sounds done; starting timeout of {} seconds'.format(
                  self._TIMEOUT_S))
    ...
    def play_sound(self, sound_event_id, connection, sender, path, iface,
        ...
        if sound_event_id not in self.metadata:
            self.info("This sound event id does not exist.",
        ...
        if overlap_behavior not in self._OVERLAP_BEHAVIOR_CHOICES:
            ...
            self.info(msg, overlap_behavior, sound_event_id=sound_event_id)
    ...
    def __player_eos_cb(self, unused_player, sound_event_id, uuid_):
        ...
        self.debug("Freeing structures because end-of-stream was reached.",
    ...
    def __player_error_cb(self, player, error, debug, sound_event_id,
        ...
        if uuid_ in self.players:
            self.debug("Freeing structures because there was an error.",
                       bus_name=self.get_player(uuid_).bus_name,
                       sound_event_id=self.get_player(uuid_).sound_event_id,
                       uuid=uuid_)
            del self.players[uuid_]
